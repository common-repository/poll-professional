jQuery(document).ready(function() {
	if (jQuery( ".sortable" ).length <= 0 || jQuery( '.poll-p' ).length <= 0) {
		return false;
	}
...
function initAddAdmin() {
	...
	jQuery('.add-poll-row').bind('click', function() {

		jQuery(this).parent().find('.answer-template').clone().appendTo('.poll-answers-container').removeClass('answer-template').hide().fadeIn();

		jQuery('.poll-p .answer-number').html(jQuery('.poll-answers-content').length - 1);
	...
	jQuery('.remove-poll-row').bind('click', function() {
...
function bindEvents() {
	...
	jQuery('.toggle-poll-row').unbind('click').bind('click', function() {

		jQuery(this).parent().find('.poll-row-toggle-area').slideToggle();

		jQuery(this).toggleClass('visible-poll-row');
	...
	jQuery('.remove-poll-row').unbind('click').bind('click', function() {

		if (confirm('Are you sute you want to delete item?')) {
...
var colorPicker = (function(){
	...
	function setActiveBaseColor(el){
		...
		jQuery(strParent).find('.color.active').removeClass('active');
	...
	function setActiveColor(el){
		...
		jQuery(strParent).find('.color-var.active').removeClass('active');
	...
	function addEventListeners(){
		...
		jQuery('.set-color').on('click', '.color', function(){

			strParent = jQuery(this).parent().parent().parent();



			var color = jQuery(this).data('color').split(',');

			setActiveBaseColor(jQuery(this));
		...
		jQuery(strParent).find('.set-color').on('click', '.color-var', function(){

			setActiveColor(jQuery(this));
	...
	function setFirstColorActive(callback){
		...
		jQuery(strParent).find('.color').eq(1).trigger('click');
	...
	function setFirstModifiedColorActive(){
		...
		setTimeout(function(){
			...
			jQuery(strParent).find('.color-var').eq(7).trigger('click');
	...
	function createColorPicker(callback){
		...
		jQuery(strParent).find('.color-picker').html('');

		jQuery(strParent).find('.color-picker').append('<div class="base-colors"></div>');

		jQuery(strParent).find('.color-picker').append('<div class="varied-colors"></div>');

		jQuery(strParent).find('.color-picker').append('<div class="active-color"></div>');

		jQuery(strParent).find('.color-picker').append('<div class="color-history"></div>');
	...
	function appendBaseColors(){
		...
		for(i = 0; i < config.baseColors.length; i++){
			...
			jQuery(strParent).find('.base-colors').append('<div class="color" data-color="' + config.baseColors[i].join() + '" style="background-color: rgb(' + config.baseColors[i].join() + ');"></div>');
	...
	function setBackgroundColor(){
		...
		jQuery(strParent).find('.set-color').css({
	...
	function createVariations(color, callback){
		...
		jQuery(strParent).find('.varied-colors').html('');
		...
		for(var i = 0; i < config.variationTotal; i++){
			...
			jQuery(strParent).find('.varied-colors').append('<div data-color="' + newColor + '" class="color-var" style="background-color: rgb(' + newColor + ');"></div>');
	...
	function setDelays(callback){
		...
		jQuery(strParent).find('.color-var').each(function(x){
	...
	function showVariations(){
		...
		setTimeout(function(){
			...
			jQuery(strParent).find('.color-var').addClass('visible').css({'transform':'none'}).bind('click', function() {

				var strColor = "rgb("+jQuery(this).data('color')+")";

				jQuery(this).parent().parent().parent().parent().find('input.color-store').val(strColor);

				jQuery(this).parent().parent().parent().parent().find('.poll-color').css({'background-color':strColor});
	...
	function hideVariations(callback){
		...
		jQuery(strParent).find('.color-var').removeClass('visible').removeClass('active');
